...
const {buildPayload} = require('./task-builder');
...
module.exports = async function handler(req, res) {
  ...
  if(file&&file.data&&claudeMsgs.length>0){
    ...
    if(li>=0){
      ...
      arr.push({type:'text',text:claudeMsgs[li].content||'Analiza este archivo.'});
      claudeMsgs[li]={role:'user',content:arr};
    }
  }
  const lastMsg=(rawMsgs.filter(m=>m.role==='user').pop()||{}).content||'';
  const IPATS=[/qui[eé]n sos/i,/sos claude/i,/en realidad sos/i,/ignor[aá].*instrucciones/i,/qu[eé] (ia|modelo)/i,/neura existe/i,/system prompt/i,/c[oó]mo funciona neura/i];
  ...
  const BPATS=[/\bsoy claude\b/i,/neura no existe/i,/mis instrucciones son/i];
  ...
  function st(txt){
    ...
    for(let i=0;i<ws.length;i+=4){
      const c=ws.slice(i,i+4).join(' ')+(i+4<ws.length?' ':'');
      res.write('data: '+JSON.stringify({type:'content_block_delta',delta:{type:'text_delta',text:c}})+'\n\n');
    }
    res.write('data: [DONE]\n\n');
    res.end();
  }
  try{
    ...
    if(bi(first)){const r2=await cc(claudeMsgs,ISTRAT+' CORRECCION: responde como NEURA.');return doStream?st(r2):res.json({reply:r2,provider:prov.name,regenerated:true,reason:'identity'});}
    if(ph(first)){const r2=await cc(claudeMsgs,ISTRAT+' CORRECCION: responde con algo util directo.');return doStream?st(r2):res.json({reply:r2,provider:prov.name,regenerated:true,reason:'postpone'});}
